<?php
  ...
  function lister($tableau) {  
          ...
          for($i=0;$i<count($tableau);$i++) {
                ...
                echo  "<tr><td><input type='checkbox' name='Choix[]'></td><td>".$tableau[$i]['nonfichier']."</td><td>".$tableau[$i]['titrefichier'].'</td><td>'.date("d m Y",strtotime($tableau[$i]['validitefichierstart'])).'</td><td>'.date("d m Y",strtotime($tableau[$i]['validitefichierend'])).'</td><td><a href="'.JURI::base().'index.php/component/mubaoz?modiffichier='.$tableau[$i]['titrefichier'].'">Modifier</a></td>';
                ?>
                <td><a href="<?php JURI::base()?>index.php/component/mubaoz?show_file=show_file&chemin='<?php echo $tableau[$i]['emplacement']; ?>'&fichier='<?php echo $tableau[$i]['titrefichier'];?>'" onclick="return confirm('Voulez-vous vraiment suprimer ce fichier?');"> Supprimer</a></td>
                <td><a href=#>Révoquer</a>
                <?php 
          }
  }
